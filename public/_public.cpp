...
{
    if(m_shmid=shmget(key_t(SHMKEYP),MAXNUMP*sizeof(st_procinfo),0666|IPC_CREAT)==-1)
    {
        if(logfile)
            logfile->write("创建共享内存失败，无法管理进程心跳");
    }
    ...
    // }
         ...
         // 把当前进程的信息填充到结构体中。
    st_procinfo procinfo(getpid(),"server1",30,time(0));

    csemp semlock;                             // 用于给共享内存加锁的信号量id。
    if (semlock.init(SEMKEYP) == false)  // 初始化信号量。
    {
