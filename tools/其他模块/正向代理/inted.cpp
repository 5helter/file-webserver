...
vector<st_route> vroute;    //代理路由的容器
...
int clientsocks[MAXSOCK];    // 存放每个socket连接对端的socket的值。由于socket是按序分配并且
...
int main(int argc,char* argv[])
{
    if(argc!=3)
    {
        ...
        printf("Sample:./inetd /myproject/inetd.log /etc/inetd.conf\n\n");
    ...
    pactive.addpinfo(30,"inted");   //设置进程心跳

    if(loadroute(argv[2])==false)
    {
        logfile.write("路由配置错误\n");
        return -1;
    }
    ...
    sigprocmask(SIG_BLOCK, &sigset, 0);  //屏蔽进程处理信号集里的内容，交给epoll
    int sigfd=signalfd(-1, &sigset, 0);     //得到信号集的描述符
    ...
    while(true)
    {
        ...
        for (int ii=0;ii<infds;ii++)
        {
            ...
            {
                if (evs[ii].data.fd==vroute[jj].listensock)     // 判断是哪个源端口有客户端连上来了
                {
...
bool loadroute(const char* inifilename) 
{
    cifile ifile;
    if(ifile.open(inifilename)==false)
    {
        return false;
    }
